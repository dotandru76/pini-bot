...
// === בחירת ביטוי אקראי (בטוח לשימוש) ===
function pick(category) {
    if (!PINI_PERSONALITY || !PINI_PERSONALITY.expressions) {
        console.warn('[ResponseBuilder] PINI_PERSONALITY not loaded correctly');
        return '';
    }
    
    const options = PINI_PERSONALITY.expressions[category];
    
    // בדיקת בטיחות: וודא שזה מערך ושיש בו איברים
...
const RESPONSE_TEMPLATES = {
    ...
    quote_added: (context) => {
        ...
        const qty = item.qty ? item.qty.toLocaleString() : '0';
        const price = item.client_price ? item.client_price.toLocaleString() : '0';
        
        // תגובה בסיסית
        let response = `${pick('excitement') || 'מעולה!'} ${qty} ${productName} ב-₪${price}`;
    ...
    quote_updated: (context) => {
        ...
        let response = `${pick('empathy') || 'אין בעיה,'} ${direction} ל-${item.qty.toLocaleString()} יחידות.\n`;
    ...
    ask_quantity: (context) => {
        ...
        const productHeb = getProductHebrew ? getProductHebrew(product) : product;
    ...
    send_quote: (context) => {
        ...
        const totalStr = total ? total.toLocaleString() : '0';
        return `מכין לך הצעת מחיר רשמית... 📄\n\n💰 סה"כ: ₪${totalStr}\n\nההצעה תהיה מוכנה תיכף!`;
    },
    
    // === תגובה למחיר יקר ===
    price_objection: (context) => {
        if (!handlePriceObjection) return "אני מבין, המחיר הוא פונקציה של האיכות.";
        ...
        const empathy = generateEmpatheticResponse ? generateEmpatheticResponse('expensive') : 'אני מבין את השיקול.';
        return empathy + '\n\n' + strategy.response;
    }
};
...
function buildResponse(type, context = {}) {
    ...
    // זהה מצב רוח והתאם
    if (context.userMessage && detectMood) {
